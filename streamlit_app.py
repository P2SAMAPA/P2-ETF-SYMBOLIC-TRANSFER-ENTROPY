...
st.set_page_config(page_title="Symbolic Transfer Entropy with Macro State", layout="wide")
...
st.markdown('<p style="text-align: center;">Conditional information flow: macro → ETF given macro level (low/medium/high)</p>', unsafe_allow_html=True)
...
st.sidebar.markdown(f"**Primary macro:** {config.PRIMARY_MACRO} | **Macro bins:** {config.MACRO_BINS}")
st.sidebar.markdown(f"**ETF bins:** {config.ETF_BINS} | **Lag:** {config.LAG}")
st.sidebar.markdown(f"**Conditional on today:** {config.USE_CONDITIONAL_ON_TODAY}")
...
def find_latest_json(files):
    json_files = [f for f in files if f.endswith('.json') and 'symbolic_te_' in f]
    if not json_files:
        return None
...
def display_universe(universe_name, uni_data, window_data, window_label):
    ...
    for idx, etf in enumerate(top3):
        with cols[idx]:
            ...
            <div class="hero-card">
                <h3>{etf['ticker']}</h3>
                <p>Symb. TE score: {etf['ste_score_norm']:.3f}</p>
                <p style="font-size:0.9rem;">raw: {etf['raw_score']:.4f}</p>
            </div>
            """, unsafe_allow_html=True)
    with st.expander(f"Full ranking for {universe_name}"):
        df_full = pd.DataFrame(list(norm_scores.items()), columns=["Ticker", "Normalized STE"])
        df_full["Raw Score"] = df_full["Ticker"].apply(lambda t: raw_scores[t])
        df_full = df_full.sort_values("Normalized STE", ascending=False)
...
with tab1:
    ...
    with st.expander("📖 Interpretation", expanded=False):
        ...
        - The macro variable (e.g., VIX) is discretised into symbols (low, medium, high).
        - For each macro symbol, we compute TE(macro → ETF) using only days when macro was in that state.
        - The score is either the **average TE across symbols** or the **TE conditional on today's macro symbol**.
...
with tab2:
    ...
    st.markdown("Choose a rolling window to inspect the symbolic TE scores.")
    for universe_name, uni_data in data["universes"].items():
        if not uni_data or not uni_data.get("all_windows"):
            st.warning(f"No window data for {universe_name}")
            continue
...
st.sidebar.markdown("---")
st.sidebar.caption("Symbolic Transfer Entropy | Macro‑state‑conditioned information flow")
